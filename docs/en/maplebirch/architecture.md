...
`ModuleSystem` is responsible for managing the registration, dependency resolution, and lifecycle of all functional modules. For full API reference (`register`, `get`, `dependencyGraph`, lifecycle methods), see [ModuleSystem API](./module-system).
