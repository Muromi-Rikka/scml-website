...
- **重置框架设置** — 将 `$options.maplebirch` 恢复为默认值，并触发框架内部 `:rest-options` 事件以同步变量模块中的选项校验（界面会立即刷新当前设置页）
...
| 重置框架设置 | 清空当前存档中的 `$options.maplebirch` 并触发 `:rest-options`，由框架重新套用默认选项；当前自定义设置页会立即重绘 |
