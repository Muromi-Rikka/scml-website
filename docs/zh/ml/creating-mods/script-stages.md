...
上述所有脚本阶段均在加载屏幕期间完成。SC2 引擎启动后，会触发 jQuery 事件，Mod 可监听这些事件以响应游戏状态。事件包括：

| 事件              | 触发时机                 |
| ----------------- | ------------------------ |
| `:storyready`     | 游戏完全启动             |
| `:passageinit`    | 新 Passage 上下文初始化  |
| `:passagestart`   | 新 Passage 开始渲染      |
| `:passagerender`  | 新 Passage 渲染完成      |
| `:passagedisplay` | 新 Passage 准备插入 HTML |
| `:passageend`     | 新 Passage 处理完成      |

完整说明与更多示例见 [API 概览](../api/index)。
